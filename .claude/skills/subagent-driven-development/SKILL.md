...
记录当前 HEAD 作为 `base_SHA`，implementer 报告的 commit SHA 作为 `head_SHA`。
...
- `full_diff` = `git diff <first_base_SHA>..<last_head_SHA>`
- `base_SHA` = 第一个任务的 base_SHA
- `head_SHA` = 最后一个任务的 head_SHA
...
主会话顺序执行每任务 TDD 循环（参考 implementer-prompt.md 的 TDD 执行链）：写测试 → RED → 实现 → GREEN → 重构 → commit → 质量门控 → 调用 `/code-review` (per-task)。逐任务串行，不跳跃。
