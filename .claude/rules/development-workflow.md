...
- **架构级浮现**：调研或阅读代码时发现需要架构决策（不限于新模块/新依赖，也包括重构中浮现的边界问题）→ 停下规划，建议使用者执行 `/brainstorm`；是否触发由使用者和模型共同判断，不设硬规则
...
| "我先看看代码再决定要不要 brainstorm/investigate" | 看代码=已经在脑补方案；brainstorm（产品+技术设计）和 investigate（bug 根因分析）必须发生在阅读之前 |
