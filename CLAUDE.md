...
├── .github/workflows/hooks.yml        # CI: deterministic tests (hooks + skills) on ubuntu
├── .claude/
...
Repo root 即 plugin root。所有 hooks 通过 `CLAUDE_PLUGIN_ROOT` 引用路径，配置在 `hooks/hooks.json` 中。Claude Code 自动发现 `skills/` 目录。
...
| **审查强制** | code-review / design-review skills 内置 prompt 模板（python/typescript/security + 设计产物审查） | 审查时 |
